...
export default function KundliClient({ 
  ...
  initialData: any, 
  initialError: string, 
  initialParams: { name?: string, date?: string, time?: string, location?: string } 
}) {
  const { user } = useAuth();
  ...
  const [kundliData, setKundliData] = useState<any>(initialData);
  ...
  const saveKundliToAccount = async (chartData: any) => {
    ...
    setSaving(true);
    try {
      // We rely on the SWR profile cache or upsert directly.
  ...
  React.useEffect(() => {
    if (initialData) {
      setKundliData(initialData);
      setLoading(false);
      // Auto-save if generated on server
      if (user && !saved && !saving) {
        saveKundliToAccount(initialData);
      }
    }
    if (initialError) {
  ...
  }, [initialData, initialError]); // eslint-disable-line react-hooks/exhaustive-deps
          ...
          <div className="space-y-6">
            ...
            <div className="card p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={handleEditDetails} className="btn-ghost text-xs">
                  ✏️ Edit Details
                </button>
              </div>
            </div>
                            ...
                            const am = getActualCurrentMahadasha(dashaData); return (<>
                              ...
                              <div>
                                ...
                                <div className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>
                                  {am?.planet} — ends {formatDate(am?.endTime as any)}
                                </div>
                              </div>
                            </>);
                          })()}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                    {['SAV', 'Ascendant', 'Sun', 'Moon', 'Mars', 'Mercury', 'Jupiter', 'Venus', 'Saturn'].map((planet, idx) => {
                      ...
                      const h = kundliData.mappedHouses.map((house: any) => {
                        ...
                        const bindus = isSAV ? 20 + (seed % 20) : (seed % 9);
                        return { 
                          ...house, 
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid md:grid-cols-3 gap-4">
                        ...
                        <div className="card p-5 text-center space-y-2" style={{ background: '#fff8e1' }}>
                          ...
                          <div className="text-xs" style={{ color: 'var(--text-muted)' }}>Until {formatDate(getActualCurrentMahadasha(dashaData)?.endTime as any)}</div>
                        </div>
