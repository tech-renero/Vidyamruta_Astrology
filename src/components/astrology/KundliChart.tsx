...
export default function KundliChart({
  ...
  const getPlanetString = (p: PlanetInfo | string): string => {
    if (typeof p === 'string') {
      return p.substring(0, 2).toUpperCase();
    }
    const name = p.name.substring(0, 2).toUpperCase();
    if (p.degree !== undefined) {
      return `${name} ${Math.floor(p.degree)}°`;
    }
    return name;
  };

  const northCenters = useMemo(() => {
    return [
      { x: s2, y: s4 },
      { x: s4, y: s / 8 },
      { x: s / 8, y: s4 },
    ...
    return (
      ...
      >
        ...
        {houses.map((house, index) => {
          const hCenter = northCenters[index];
          if (!hCenter) return null;

          const houseNum = house.houseNumber ?? index + 1;
          const rashiLabel = house.rashi !== undefined ? rashiNames[house.rashi] : rashiNames[index];

          return (
            <g key={`north-${index}`}>
              <text x={hCenter.x} y={hCenter.y - 14} textAnchor="middle" fontFamily="Inter, sans-serif">
                <tspan fontSize={width > 300 ? '9' : '7'} fill="#9e9e9e" fontWeight="500">
                  {houseNum}{' '}
                </tspan>
                <tspan fontSize={width > 300 ? '12' : '10'} fill="#e65100" fontWeight="800">
                  {rashiLabel}
                </tspan>
              </text>
              <text
              ...
              >
                {house.planets.map((p, i) => (
                  <tspan x={hCenter.x} dy={i === 0 ? 0 : 12} key={`${index}-${i}`}>
                    {getPlanetString(p)}
                  </tspan>
                ))}
              </text>
        ...
        {houses.map((house, index) => {
          ...
          return (
            <g key={`south-${index}`}>
              ...
              >
                {house.planets.map((p, i) => (
                  <tspan x={centerX} dy={i === 0 ? 0 : 12} key={`${index}-${i}`}>
                    {getPlanetString(p)}
                  </tspan>
                ))}
              </text>
